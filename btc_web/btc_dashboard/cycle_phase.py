...
三层结构:
  1. 温度计 (thermometer): 反转侧因子合成 (趋势伸展 + 链上筹码 + 减半钟)。
     已验证四轮周期顶部全部 ≤-0.5、底部 ≥+0.6 — 是"周期位置"的
     鲜明信号; 但历史上顶部前 1-7 个月即开始报警, 直接当仓位分用回测
     Sharpe 0.73 vs 现行 1.00 — 故只做判读输入, 不驱动仓位。
...
THERMOMETER_BUCKETS = {
    ...
    "链上筹码": {
        "weight": 0.45,
        # 交易所余额曾在此先行剔除 (ETF 时代结构性偏多 + 与 ETF净流入双重计数);
        # 2026-07 留一对照后它已全面退出周期评分, 本桶与 scoring 链上筹码桶现一致
