...
def halving_band(months: float) -> float:
    """减半时钟先验分档 — 全库唯一事实源 (indicators_long / backfill / backtest 三处共用)。

    档位依据 2013-2026 分段前瞻收益 (2026-07 证据回测, 样本内标定):
      0-12月  牛市段 (+1): 365d 前瞻中位 +259%, 82% 为正
      12-24月 顶部与崩塌段 (-1): 365d 中位 -47%, 仅 15% 为正 — 全表最差段;
              C1-C3 皆顶部+崩塌, C4(本周期)亦复刻 (2025-H2 见顶/2026-H1 崩塌), n=4。
              2026-07 由 0 改 -1: 留一对照 IC365 0.562→0.603 (post21 0.513→0.597),
              Sharpe 1.09→1.16, 回撤 -37.9%→-35.3% (变体 -0.5 被 -1 支配)
      24-30月 深熊窗 (-1): C1-C3 段内价格皆 -37%~-40%, n=3 仅先验
      >30月   复苏/积累段 (+0.5): C1-C3 段内 +118%~+277%, 365d 前瞻 95-100% 为正;
              旧版此段延续 -1 属反信号 (2026-07 修正)。+0.5 而非 +1: n=3 先验保持谦逊
    """
    if months <= 12:
        return 1
    if months <= 30:
